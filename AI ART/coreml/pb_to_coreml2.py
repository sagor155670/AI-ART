...
def parse_args():
    ...
    parser.add_argument('-i','--pb_model_path', type=str, default="/mnt/data/xinchen/temp/AnimeGANv3_Shinkai.pb", help='The path of the input pb file')
    parser.add_argument('-o','--out_ml', type=str,  default="/mnt/data/xinchen/temp/coreml",help='The path of the output mlmodel file')
    parser.add_argument('-iw','--input_w', type=int, default=1024, help='The width of input image')
    parser.add_argument('-ih','--input_h', type=int, default=1024, help='The height of input image')
...
def toml(pbfile, modelname, input_w, input_h): # in AnimeGANv3 the name is : AnimeGANv3_input or animeganv3_input. the name key-value can be omitted.
    ...
    builder.add_permute(name="permute", dim=[0, 3, 1, 2], input_name= "generator_output", # "generator_main_out_layer" ( output name of v3 face style)
...
def main():
    ...
    os.makedirs(args.out_ml, exist_ok=True)

    if args.pb_model_path.endswith(".pb"):
        print(os.path.join(args.out_ml, args.pb_model_path.replace("pb", "mlmodel")))
        toml(args.pb_model_path, os.path.join(args.out_ml, args.pb_model_path.replace("pb", "mlmodel")), args.input_w,args.input_h)

    else:
        for x in os.listdir(args.pb_model_path):
            if x.endswith(".pb"):
                try:
                    toml(os.path.join(args.pb_model_path,x) , os.path.join(args.out_ml,x.replace("pb", "mlmodel")) ,args.input_w , args.input_h)
                except:
                    print(x)
